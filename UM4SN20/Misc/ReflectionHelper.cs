...
namespace UM4SN.Misc
{
    public static class ReflectionHelper
    {
        ...
        {
            NotNull(fieldInfo, $"Field cannot be null!");
            return fieldInfo.GetValue(o);
        }
        ...
        public static object ReflectionGetCast<T, T2>(this T2 o, string fieldName, bool isPublic = false, bool isStatic = false)
            where T2 : T
        {
            ValidateStatic(o, isStatic);
            Type t = typeof(T);
        ...
        private static MethodInfo GetMethod(this Type t, string methodName, bool isPublic = false, bool isStatic = false, params Type[] types)
        {
            ...
            if (types != null && types.Length > 0)
            {
                ...
                NotNull(methodInfo, $"Type \"{t.Name}\" does not have a method called \"{methodName}\", with bindingFlags {bindingFlags} and types {string.Join(", ", types.Select(typ => typ.ToString()).ToArray())}.");
            }
            else
            {
                methodInfo = t.GetMethod(methodName, bindingFlags);
                NotNull(methodInfo, $"Type \"{t.Name}\" does not have a method called \"{methodName}\" with bindingFlags {bindingFlags}.");
            }
            return methodInfo;
        }
        ...
        private static FieldInfo GetField(this Type t, string fieldName, bool isPublic = false, bool isStatic = false)
        {
            ...
            NotNull(fieldInfo, $"Type \"{t.Name}\" does not have a field called \"{fieldName}\" with bindingFlags {bindingFlags}.");
            return fieldInfo;
        }
        ...
        private static void NotNull<T>(T o, string message)
            where T : class
        {
            if (o == null)
            {
